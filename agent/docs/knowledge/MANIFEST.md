...
| `odata-metadata-cache.md` | FileMaker Server's OData service caches the schema on startup. After schema changes (field add/drop/rename, validation, primary-key composition), the OData cache holds the old shape and clients see inconsistent behavior — silent write failures, missing fields in `$select`, scripts called via `Script.{name}` returning ERROR with confusing messages that don't reproduce in FM Pro. Bounce the OData service to flush the cache. | OData, metadata, cache, schema change, FM Server, field add, field drop, field rename, validation, primary key, unique, restart, bounce, hot environment, deploy, silent failure, missing field, Script.name POST, $metadata, hosted, integration |
