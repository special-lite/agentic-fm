...
- **Script step reference**: `agent/docs/filemaker/script-steps/` — official docs for step options, compatibility, and behavioral notes. Complements the step catalog, which covers XML structure but not semantics.
- **Guides**: `agent/docs/filemaker/guides/<doc-set>/` — whole Claris guides when present. `sql-reference` for `ExecuteSQL` / `ExecuteSQLe` syntax and supported grammar, `odata-guide` for OData request shapes and limits, `data-api-guide` / `admin-api-guide` for the REST APIs, `security-guide` for privilege sets. Consult these before reasoning from memory about SQL or OData behavior.
- **Generating the docs**: all of the above are gitignored and produced by `python3 agent/docs/filemaker/fetch_docs.py` (stdlib only). If a folder is missing, tell the developer the command rather than guessing at syntax. `--all --refresh` updates an existing corpus cheaply.
